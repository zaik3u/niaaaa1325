...
int altoPantalla = 24;
char fechaActual[] = "2025-06-13 00:44:55";
...
struct ventas {
    ...
    int cantidad;
    float precioVenta;
    char fecha[20];          // Cambiado a 20 para consistencia
    char usuario[30];        // Nuevo campo - usuario que registra
};
...
struct clientes {
    ...
    char nombre[30];
    char ciudad[20];
    char telefono[15];       // Nuevo campo
    char direccion[30];      // Nuevo campo
    char fechaRegistro[20];  // Nuevo campo
};
...
int buscarProducto(int codigo, struct productos *producto) {
    ...
    while(fread(&temp, sizeof(temp), 1, archivo)) {
        if(temp.codigo == codigo) {
            *producto = temp;  // Copia todos los datos al puntero proporcionado
            encontrado = 1;
            break;
        }
    }
...
void iniciarSesion() {
    ...
    while(intentos < maxIntentos && !acceso) {
        ...
        gotoxy(25, 10);
        printf("? Tienda De Bayas Sovieticas ?");
        gotoxy(20, 12);
        printf("Camarada: ");
        gotoxy(20, 13);
...
void menuPrincipal() {
    ...
    do {
        ...
        gotoxy(21, 11);printf("3  Registrar Importacion");
        ...
        gotoxy(20, 20);printf("Seleccione una opcion, Camarada (0-9): ");
...
void registrarProducto() {
    clrscr();
    system("color 2F");  // Fondo verde, texto blanco brillante
    dibujarMarcoSovietico();
...
void registrarVenta() {
    clrscr();
    system("color 4F");  // Fondo rojo, texto blanco brillante
    dibujarMarcoSovietico();
...
void registrarCliente() {
    clrscr();
    system("color 6F");  // Fondo marrón, texto blanco brillante
    dibujarMarcoSovietico();
